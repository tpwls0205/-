...
namespace _63_NetworkStreamMultiThreadServerForm
{
    public partial class Form1 : Form
    {
        ...
        bool isRecv = true;     // 연결담당 스레드 반복 플래그

        /* Main 스레드가 아닌 Sub 스레드에서 윈폼의 컨트롤을 직접 접근하면 GUI스레드가 사용 중 일때는
         * 예외가 발생하게 된다. 그러므로 사용 중 일때는 일단 메서드 호출을 .NET 프레임워크에 통보를
         * 전달 해놓으면 .NET이 컨트롤 접근이 안전할 때 자동으로 호출해준다.
         * <요약> 별도 스레드에서 윈폼의 컨트롤을 직접 접근하면 예외가 발생한다.
         */
        ...
        private void btnStart_Click(object sender, EventArgs e)
        {
            ...
            this.ipep = new IPEndPoint(IPAddress.Any, Int32.Parse(tbPort.Text));
            ...
            btnStart.Enabled = false;       // 버튼비활성화
            btnStop.Enabled = true;         // 버튼활성화
        }
        ...
        /* Main스레드에서는 AddLogListBox메서드 호출이 문제 없는데 Sub스레드에서는 아래처럼
         * Invoke처리를 해줘야 한다.
         */
        void AddLogListBox(string data)     // 서브 스레드에서 호출할 때 오류방지
        {
            // 직접 컨트롤을 접근할 수 없는 상황일 때
            if(lbLog.InvokeRequired)                        // 간접호출
            {
                // .NET에 등록처리를 한다(나중에 자동으로 addMsgData가 호출)
                Invoke(addMsgData, new object[] { data });  // 자기자신을 호출
            }
            else
            {
                lbLog.Items.Add(data);                      // if문 한번 다녀온 후 else에서 데이터 추가
        ...
        void ThreadAccept()
        {
            while(this.isAccept)
            {
                ...
                try
                {
                    Socket parterSocket = this.acceptSocket.Accept();
                    AddLogListBox("클라이언트 접속");
                    NetworkStream ns = new NetworkStream(parterSocket);
