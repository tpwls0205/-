...
namespace _12_MouseSelectEllipseMove
{
    public partial class Form1 : Form
    {
        ...
        private void Form1_MouseDown(object sender, MouseEventArgs e)
        {
            ...
            if (e.Button == MouseButtons.Left)
            {
                // 원의 중심좌표와 마우스 클릭 위치를 전달해서 원 안에 있는지 판단하는 메서드
        ...
        private void Form1_Load(object sender, EventArgs e) // 객체가 다 생성되고 초기화 상태
        {
            this.BackColor = Color.Black;
            this.DoubleBuffered = true;                     // 화면깜빡임을 없애줌
        }

        private void Form1_Paint(object sender, PaintEventArgs e)
        {
            Graphics g = e.Graphics;
            Pen myPen = new Pen(Color.DeepSkyBlue, 10);
        ...
        /// <summary>
        /// 원의 중심으로부터 마우스 클릭 위치를 계산하여 원 안에 클릭했는지
        /// 원 밖을 클릭했는지 판단하는 메서드
        /// </summary>
        /// <param name="centX"> 원의 중심 x 좌표 </param>
        /// <param name="centY"> 원의 중심 x 좌표 </param>
        /// <param name="mouseX"> 마우스 클릭 x 좌표 </param>
        /// <param name="mouseY"> 마우스 클릭 x 좌표 </param>
        /// <returns>
        /// true면 원 내, false면 원 밖
        /// </returns>
        ...
        {
            double result = 0;
            result = Math.Sqrt((x1 - x0) * (x1 - x0) + (y1 - y0) * (y1 - y0));
            return result;
        }
    }
}
