/*	static 지역변수
	...
	3. 다른 지역에 있는 static 지역변수와 이름이 달라야 한다
	4. 외부에서는 지역변수이고 내부에서는 전역변수이다
	5. 다른 함수에서 접근을 못한다(지역변수 속성)
	6. 프로그램이 실행 시 생성
		프로그램 종료 시 소멸
		값을 계속 유지할 수 있다
*/
...
void main()
{
	...
	while (1)
	{
		preTime0 = runFirst("▣▣▶", 5L, preTime0);	// 100ms -> 0.1초마다 움직임
		preTime1 = runSecond("★☆@", 10L, preTime1); // 200ms -> 0.2초마다 움직임
		preTime2 = runThird("◎♧♡", 15L, preTime2); // 200ms -> 0.2초마다 움직임
	}
}