...
void inputAddress(Address* pAddr)
{
	...
	pAddr->name[strlen(pAddr->name) - 1] = 0;	// fgets의 엔터값 제거용도
	printf("주소 입력 : ");
	fgets(pAddr->address, sizeof(pAddr->address) - 1, stdin);
	pAddr->address[strlen(pAddr->address) - 1] = 0;	// fgets의 엔터값 제거용도
	printf("나이 입력 : ");
	scanf_s("%d", &(pAddr->age));
	getchar();	// \n 제거용도
}
...
{
	int num = 0;
	printf("당신은 몇명의 주소를 저장하시겠습니까? ");
	scanf_s("%d", &num);
	getchar();  // \n 제거용도

	// Address배열을 num크기만큼 힙메모리에 할당하고
	// 배열의 시작주소를 pAddr에 저장하였다.
	Address* pAddr = (Address*)malloc(sizeof(Address)* num);
