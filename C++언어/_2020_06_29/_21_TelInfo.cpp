...
/*	C++에서 struct와 class는 99% 같다
	단 1가지 차이가
	struct는 기본적으로 모두 public접근 권한
	class는 기본적으로 모두 private접근 권한이다.
	이렇게 차이가 나는 이유는?
	1. C++에서 C코드를 실행할 수 있게 하기 위해서(코드 호환성을 갖게 하려고)
	C언어에서 구조체의 멤버는 그냥 접근한다(public)
	2. class의 기본권한을 private으로 하려고 했던 이유는
	전역변수를 이 함수 저 함수에서 마구 접근하는 관계로 예상치 못한
	버그가 많이 발생하는 것을 경험
	그래서 변수는 막 접근하게 하면 안되는구나!
	정식절차를 밟은 대상만 접근을 허용하게 하려고 한다.
	그래서 기본적으로 변수를 보호하는 것이 중요하다고 생각했다.
*/
class Person
{	
...
// C++에서는 이런 함수를 전역 함수라 부른다. 클래스 안에 있는 함수는 멤버함수
void ShowMenu();
