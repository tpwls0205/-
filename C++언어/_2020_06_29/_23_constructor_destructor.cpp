/*	클래스를 설계할 때
	외부에서 관심을 갖지 말아야 할 것(호출될 필요가 없는 멤버는)
	private으로 감춰준다. => 캡슐화한다.
*/
#include <iostream>
#include <string>
...
{
private:
	string cooks[6] = { "쇠고기덮밥", "카레덮밥", "떡볶이", "순대", "오뎅탕", "튀김" };
	int money = 1000000;
	float meat;
	float rice;
	int selCookNum;
...
public:
	/*	
		...
		클래스명과 동일하다
	*/
	// 객체 변수 생성시 자동호출(초기화 작업)
	Restaurant()
	{
		cout << "생성자 함수 호출" << endl;
		prepareRestaurant(100000, 2.0f, 2.0f);
	}
