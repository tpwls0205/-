...
void main()
{
	...
	int* arr = new int[allocSize];	// 동적배열
	int cnt = 0;					// 입력 개수
	...
	while (1)
	{
		...
		if (cnt == allocSize)
		{
			...
			if (answer == "yes" || answer == "y" || answer == "Y" || answer == "YES")
			{
				...
				//memcpy(tempArr, arr, sizeof(int) * (allocSize - ALLOC_SIZE));	// arr시작주소에서 크기만큼 tempArr로 복사
				// 3. 기존 힙 할당공간을 해제
				delete[] arr;
				// 4. 새로운 공간의 포인터주소를 기존 포인터에 복사
				arr = tempArr;
			}
			else
			{	
